...
/**
 * Drops today's chip cache — called when the suggestions PROMPT is edited:
 * chips generated by the old prompt must not linger as today's "new" answer.
 */
export function clearDailySuggestions(): void {
  db.delete(appSettings).where(eq(appSettings.key, SUGG_KEY)).run();
}
