...
- **Need an occurrence's `originalDueAt` for anything (ICS UID, an audit trail)?** Use `occurrenceDetailsInRange()`, not `occurrencesInRange()` — the latter only returns display dates (`plannedFor`) and deliberately throws away which occurrence is "current" vs. future-grid. Both share one algorithm; don't reimplement the grid walk a third time.
...
- **The Docker runner stage must also copy `tsconfig.json`** (one `COPY` line, PR #54). `scripts/migrate.ts` runs under `tsx`, which resolves the `@/*` alias via the tsconfig (`get-tsconfig`) — invisible until the first `@/` import entered `schema.ts`'s import graph (PR #53), then every container boot died with `Cannot find module '@/lib/domain/action-types'` before the app started. Anything new that `migrate.ts` pulls in transitively now needs to exist in the runner stage.
...
- `action-types.ts` is the single source of truth for the whole app (schedules, logs, UI, MCP, `/api/v1/*`) — never maintain a divergent type list elsewhere. API specifics: `POST /api/v1/actions` accepts only the catalog's `LOGGABLE_ACTION_TYPES` (everything except `feed`); `feed` gets a dedicated 400 redirecting to `POST /api/v1/tanks/{id}/feedings` (daily counter, `delta: 1|-1`). When a log call omits `detailData`, `logActionCore` inherits the structured details from the tank's matching active plan — so display/HA clients can send bare `{actionType}` and still get full history rows.
