...
- Every AI-proposed write requires explicit human confirmation in UI (approval gate is the security boundary in this auth-less v1). Two shapes: chat proposals go through `applyProposal` with an approval card; single-purpose DRAFT modules (`product-draft.ts`, `feeding-plan-draft.ts`) skip the card on purpose — one provider call, one tool, zod, result lands in a FORM/editor and the human's Save is the gate. New "draft me X" features copy that module shape (purpose-tagged `aiCalls`, failure codes split fine, pre-provider checks that cost zero tokens)
